...
const ICONS = {
  ...
  emergency: (label: string) =>
    L.divIcon({
      ...
      className: "dt-icon-wrap", iconSize: [30, 30], iconAnchor: [15, 15],
    }),
};
...
// ------- Route line styling -------
const routeStyles: Record<string, { color: string; weight: number; dashArray?: string; opacity?: number; className?: string }> = {
...
const heatColor = {
  safe: "#3ddc84", warn: "#f1c453", risk: "#ff8b3d", critical: "#ff4d4d", predict: "#b06bff",
};
...
export default function DigitalTwinMap() {
  ...
  const layers = currentStep?.layers ?? {
    immersion: true, cameras: true, emergency: true, routes: true, crowd: true, cranes: true,
  };
    ...
    <div className="relative flex-1 rounded-lg overflow-hidden border border-[var(--panel-border)]">
      ...
      >
        ...
        {currentStep?.heatZones?.map((z, i) => (
          <Circle
            ...
            pathOptions={{
              ...
              fillOpacity: z.predicted ? 0.10 : 0.18,
        ...
        {showAllRoutes && allRoutes.map((r) => (
          <Polyline key={r.id} positions={r.coordinates as any} pathOptions={{ color: "#4d6ba0", weight: 2, opacity: 0.35 }} />
        ))}
        {!showAllRoutes && allRoutes.map((r) => {
          ...
          return (
            <Polyline
              ...
              pathOptions={{ color: s.color, weight: s.weight, opacity: s.opacity, dashArray: s.dashArray, className: s.className }}
            >
              <Popup>{r.name} · {style}</Popup>
            </Polyline>
          );
        })}

        {/* Immersion Points */}
        {layers.immersion && ips.map((ip) => (
          <Marker
            key={ip.id} position={ip.coordinates as any}
            icon={focus.has(ip.id) ? ICONS.ipFocus(ip.id.split("-")[1]) : ICONS.ip(ip.id.split("-")[1])}
          >
            <Popup>
              <div className="font-semibold text-[13px]">{ip.id} · {ip.name}</div>
              <div className="text-[11px] opacity-80 mt-1">
                Status: <b>{ip.status}</b><br />
                Crowd: <b>{ip.current_crowd}</b> / {ip.safe_crowd_capacity} ({Math.round(ip.current_crowd/ip.safe_crowd_capacity*100)}%)<br />
                Cranes: <b>{ip.available_cranes}</b> / {ip.total_cranes}<br />
                Queue: <b>{ip.queue_length}</b> · Avg wait: ~{ip.average_service_time * Math.max(1, ip.queue_length/2)} min
              </div>
            </Popup>
          </Marker>
        ))}
        ...
        {layers.cranes && cranes.map((c, i) => {
          ...
          const bg = c.status === "FAULT" ? "var(--color-critical)"
                   : c.status === "MAINTENANCE" ? "var(--color-warn)"
                   : c.status === "BUSY" ? "#5a7cff"
                   : "var(--color-safe)";
          return (
            <Marker key={c.id} position={pos as any} icon={ICONS.crane(bg)}>
              <Popup>
                ...
                <div className="text-[11px] opacity-80 mt-1">
                  ...
                  Max weight: {c.max_supported_weight} kg<br />
                  Max height: {c.max_supported_height} ft<br />
        ...
        {layers.cameras && cams.map((c) => (
          <Marker key={c.id} position={c.coordinates as any}
            ...
            <Popup><b>{c.name}</b></Popup>
          </Marker>
        ))}

        {/* Emergency services */}
        {layers.emergency && ems.map((e) => (
          <Marker key={e.id} position={e.coordinates as any}
            icon={e.kind === "hospital" ? ICONS.hospital() : e.kind === "police" ? ICONS.police() : ICONS.ambulance()}>
            <Popup><b>{e.name}</b></Popup>
          </Marker>
        ))}
        ...
        {initializing && (
          ...
          >
            <div className="text-center space-y-2 max-w-md">
              <div className="text-[11px] uppercase tracking-[0.24em] text-[var(--color-info)]">Initializing Digital Twin</div>
...
function LegendRow({ color, label, dash }: { color: string; label: string; dash?: boolean }) {
  return (
    <div className="flex items-center gap-2">
      <span className="w-3 h-3 rounded-sm inline-block" style={{ background: dash ? "transparent" : color, borderTop: dash ? `2px dashed ${color}` : undefined }} />
      <span>{label}</span>
    </div>
  );
}

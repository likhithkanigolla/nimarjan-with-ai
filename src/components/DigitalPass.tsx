// A visually distinctive Digital Immersion Pass card used by Scenario 3.
...
export function DigitalPass() {
  ...
  const statusColor = reallocated ? "var(--color-safe)"
    : failed ? "var(--color-critical)"
    : allocated ? "var(--color-info)" : "var(--color-warn)";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 relative">
        <div>
          <div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Digital Immersion Pass</div>
          <div className="text-lg font-semibold font-mono mt-0.5">GNI-2026-0142</div>
          <div className="text-xs text-muted-foreground">Sri Vinayaka Youth Association · PDL-007</div>
        </div>
      ...
      <div className="mt-3 flex items-center justify-between relative">
        ...
        <span className="text-[10px] text-muted-foreground">Pass adapts to live Digital Twin state</span>
      </div>
    </motion.div>
  );
}

function Row({ k, v, tone }: { k: string; v: string; tone?: "info"|"safe"|"warn"|"critical" }) {
  const c = tone === "safe" ? "var(--color-safe)"
  ...
  return (
    <>
      ...
      <div className="text-right font-mono" style={{ color: c }}>{v}</div>
    </>
  );
}
